...
class YOLOLoss(nn.Module):
    ...
    def forward(self, l, input, targets=None):
        ...
        #   targets ground truth [batch_size, num_gt, 5]
        # ----------------------------------------------------#
        # --------------------------------#
        #   batch size，feature wh
        # --------------------------------#
        bs = input.size(0)
        in_h = input.size(2)
        in_w = input.size(3)
        # -----------------------------------------------------------------------#
        #   calculation step
        #   each feature point corresponds to pixels in image
        #   
        #   13x13 feature layer -> 1 point = 32x32 pixels
        #   26x26 feature layer -> 1 point = 16x16 pixels
        #   52x52 feature layer -> 1 point = 8x8 pixels
        ...
        # -----------------------------------------------#
        #   adjustment parameters of pbox's cx, cy
        # -----------------------------------------------#
        x = torch.sigmoid(prediction[..., 0])
        y = torch.sigmoid(prediction[..., 1])
        # -----------------------------------------------#
        #   adjustment parameters of pbox's wh
        # -----------------------------------------------#
        w = prediction[..., 2]
        h = prediction[..., 3]
        # -----------------------------------------------#
        #   confidence, object or not
        # -----------------------------------------------#
        ...
        # ---------------------------------------------------------------#
        #   calc overlap between pred and gt
        #   overlap too large, ignore it. because it is good feature points
        #   not suitable for negative samples
        # ----------------------------------------------------------------#
